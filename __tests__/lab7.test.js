describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    const prodItemsData = await page.$$eval('product-item', prodItems => {
      return prodItems.map(item => {
        ...
        let data = item.data;
        return {title: data.title, price: data.price, image: data.image};
      });
    });
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      if (!item.title || !item.price || !item.image) {
        ...
        console.log(`Item ${i + 1} is missing data:`, item);
      }
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountHandle = await cartCountElem.getProperty('innerText');
    const cartCountText = await cartCountHandle.jsonValue();
